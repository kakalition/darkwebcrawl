...
class DarkwebCrawler(BaseCrawler):
    ...
    def _get_thread_topic(self, soup):
        ...
        thread_topic = soup.find("div", {"class": "logo"})
        board_name = thread_topic.find("br").next_sibling  # Mengambil text setelah tag <br>
        if board_name:
    ...
    def _scrape_post(self, post, i):
        try:
            ...
            if not thread_topic:
                ...
                thread_topic_text = thread_topic

            thread_section = self._get_thread_section(self.driver.current_url)

            # Find post elements with null checks
            poster = post.find("span", {"class": re.compile(r"postername")})
            poster_text = poster.text if poster else "Anonymous"

            # Find timestamp that comes after postername span
            published_at = "-"
            if poster:
                # Get the next sibling text node after the postername span
                timestamp_text = poster.next_sibling
                if timestamp_text:
                    # Clean up the timestamp text
                    timestamp_str = timestamp_text.strip()
                    try:
                        # Parse the timestamp in format "DD/MM/YY(Day)HH:MM:SS"
                        # Example: "24/12/08(Sun)23:56:16"
                        parsed_date = datetime.datetime.strptime(timestamp_str, "%y/%m/%d(%a)%H:%M:%S")
                        published_at = parsed_date
                    except ValueError as e:
                        logging.error(f"Error parsing datetime: {e}")

            media = post.find('a', href=True, class_='imgLink')
            content_div = post.find('div', {"class": "message"})
            content_text = clean_html(content_div.text) if content_div else ""
            
            # Updated post ID generation using post_op or post_reply
            post_op = post.find('div', class_='op')
            post_reply = post.find('td', class_='reply')
