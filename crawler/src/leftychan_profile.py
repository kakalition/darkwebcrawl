...
class DarkwebCrawler(BaseCrawler):
    ...
    def _get_member_username(self, post):
        ...
        try:
            # Find the intro section which contains the username
            intro = post.find("p", class_="intro")
            if intro:
                # Look for the name span directly in the intro
                name_span = intro.find("span", class_="name")
                if name_span and "anonymous" not in name_span.text.lower():
    ...
    def _get_avatars(self, post):
        ...
        try:
            img_link = post.find('img', class_="post-image")
            if img_link and img_link.get('src'):
                return urljoin(self.base_url, img_link['src'])
            return "-"
        except Exception as e:
    ...
    def _get_posts(self, soup):
        ...
        try:
            ...
            div = soup.find_all('div', class_='thread')
